...
"${text}"`;
    ...
    console.info(
      `${EXTENSION_NAME}: [Anthropic] Stage 1 - Result: ${object.hasClaim ? 'HAS CLAIMS' : 'NO CLAIMS'} (tokens: ${usage.totalTokens})`
    );

    if (object.hasClaim && object.claims.length > 0) {
      console.info(
        `${EXTENSION_NAME}: [Anthropic] Stage 1 - Claims found: ${object.claims.join('; ')}`
      );
    }
...
- Be conservative - prefer "unknown" when uncertain`,
      tools: {
        web_search: anthropic.tools.webSearch_20250305({
          // Anthropic's built-in Brave Search integration
        }),
      },
      temperature: 0.5,
    });

    console.info(`${EXTENSION_NAME}: [Anthropic] Stage 2 - Web search and analysis complete`);
    ...
    try {
      ...
      console.error(
        `${EXTENSION_NAME}: [Anthropic] Failed to parse verification result:`,
        error
      );
      console.error(`${EXTENSION_NAME}: [Anthropic] Raw response:`, result.text);
    ...
    console.info(
      `${EXTENSION_NAME}: [Anthropic] Stage 2 - Verdict: ${verdictResult.verdict} (confidence: ${verdictResult.confidence}%)`
    );
